...
export const login = handleAsync(async (req, res) => {
  let existingUser = await UserInstance.getOneLeanDoc(
    {
  ...
  // console.log('---', await UserInstance.matchPassword(
  //   req.body.password,
  //   existingUser.password
  // ));
  if (
    !(await UserInstance.matchPassword(
      req.body.password,
      existingUser.password
    ))
  ) {
  ...
  return res.success({
    ...
    token: await UserInstance.genToken(existingUser._id, existingUser.email),
